...
fun TeamMemberItem(member: TeamMember) {
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text(text = stringResource(id = member.name), style = MaterialTheme.typography.titleMedium)
            Text(text = stringResource(id = member.description), style = MaterialTheme.typography.bodyMedium)
        }
    }
}